...
import type { AuthenticatedRequest, CouncilResponse, CouncilMemberResponse } from '../types/index.js';
...
import {
  ...
  addMemberSchema,
  updateMemberSchema,
  councilIdParamSchema,
  memberAddressParamSchema,
...
import {
  ...
  TermsRegistryAbi as TermsRegistryABI,
  ERC8004RegistryAbi as ERC8004ABI,
  publicClient,
...
router.get('/', async (_req, res: Response) => {
  try {
    ...
    const councils: CouncilResponse[] = [];
...
router.get(
  ...
  async (req, res: Response) => {
    try {
      ...
      const members: CouncilMemberResponse[] = [];
...
router.get(
  ...
  async (req, res: Response) => {
    try {
      ...
      const ERC8004_REGISTRY = CONTRACTS.erc8004Registry as Address;
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const member = await createCouncilMember(
        councilId,
        address,
        name,
        description,
...
router.put(
  ...
  requireAuth,
  requireSafeOwner,
  validateParams(memberAddressParamSchema),
