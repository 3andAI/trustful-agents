...
const templates: Record<EmailTemplate, (vars: Record<string, string>) => EmailContent> = {
...
export async function queueEmail(
  ...
  const result = await queryOne<EmailQueueItem>(
    `INSERT INTO email_queue (recipient_email, template, variables, scheduled_at)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
    [recipientEmail, template, JSON.stringify(variables), scheduledAt || new Date()]
  );
...
export async function getPendingEmails(limit = 10): Promise<EmailQueueItem[]> {
  return queryMany<EmailQueueItem>(
...
export async function notifyAllSigners(
  template: EmailTemplate,
  variables: Record<string, string>,
  excludeAddress?: string
): Promise<number> {
  // Get all governance signers with emails
  const signers = await queryMany<{ email: string }>(
    `SELECT email FROM governance_signers 
     WHERE email IS NOT NULL 
     ${excludeAddress ? 'AND address != $1' : ''}`,
...
export async function notifyCouncilMembers(
  ...
  const members = await queryMany<{ email: string }>(
    `SELECT email FROM council_members 
     WHERE council_id = $1 AND email IS NOT NULL
     ${excludeAddress ? 'AND address != $2' : ''}`,
    excludeAddress ? [councilId, excludeAddress.toLowerCase()] : [councilId]
  );
