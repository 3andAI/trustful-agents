 ...
 * Prepare agent reassignment (returns transaction data for Safe)
 */
router.post('/:id/reassign', requireAuth, requireSafeOwner, validateParams(agentIdParamSchema), validateBody(reassignAgentSchema), async (req, res) => {
    try {
        const agentId = BigInt(req.params.id);
