...
import { validateBody, validateParams, proposeTransactionSchema, safeTxHashParamSchema, } from '../middleware/validation.js';
import { getSafeInfo, getPendingTransactions, getTransaction, } from '../services/safe.js';
 ...
 */
router.get('/pending', requireAuth, requireSafeOwner, async (_req, res) => {
    try {
 ...
 */
router.get('/transactions/:safeTxHash', requireAuth, requireSafeOwner, validateParams(safeTxHashParamSchema), async (req, res) => {
    try {
 ...
 */
router.post('/propose', requireAuth, requireSafeOwner, validateBody(proposeTransactionSchema), async (req, res) => {
    try {
        const { to, data, value, description } = req.body;
        // Log the proposal intent
        await logAuditEvent('safe_tx_proposed', req.session.address, 'safe_tx', 'pending', // Will be updated with actual hash after client-side proposal
 ...
 */
router.post('/sign/:safeTxHash', requireAuth, requireSafeOwner, validateParams(safeTxHashParamSchema), async (req, res) => {
    try {
        const { safeTxHash } = req.params;
        ...
        // Log the signing
        await logAuditEvent('safe_tx_signed', req.session.address, 'safe_tx', safeTxHash, { description: tx.description });
        res.json({
 ...
 * This endpoint logs the action for audit purposes.
 */
router.post('/execute/:safeTxHash', requireAuth, requireSafeOwner, validateParams(safeTxHashParamSchema), async (req, res) => {
    try {
        const { safeTxHash } = req.params;
        // Verify transaction exists
        const tx = await getTransaction(safeTxHash);
        ...
        // Log the execution
        await logAuditEvent('safe_tx_executed', req.session.address, 'safe_tx', safeTxHash, { description: tx.description });
        res.json({
