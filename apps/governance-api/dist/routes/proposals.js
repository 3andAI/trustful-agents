...
import { z } from 'zod';
import { createProposal, getProposal, getProposals, getPendingProposals, castVote, getVote, getVotesForProposal, checkAndResolveProposal, hasPendingProposal, updateProposalStatus, } from '../services/proposals.js';
...
function formatProposalResponse(proposal, myVote) {
    if (!proposal)
        return null;
    return {
        id: proposal.id,
...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        const proposals = await getProposals(status, type, councilId);
 ...
 */
router.get('/:id', requireAuth, validateParams(proposalIdParamSchema), async (req, res) => {
    try {
 ...
 */
router.post('/create-council', requireAuth, requireSafeOwner, validateBody(createCouncilSchema), async (req, res) => {
    try {
        const { name, description, vertical } = req.body;
        // Get Safe threshold for approval requirement
        const safeInfo = await getSafeInfo();
        // Create proposal
        const proposal = await createProposal('create_council', req.session.address, safeInfo.threshold, {
            councilName: name,
            councilDescription: description,
            councilVertical: vertical,
        });
        // Log audit
        await logAuditEvent('council_created', req.session.address, 'council', proposal.id, { name, description, vertical, status: 'proposed' });
        // Notify other signers
        await notifyAllSigners('vote_required', {
 ...
 */
router.post('/delete-council', requireAuth, requireSafeOwner, validateBody(deleteCouncilSchema), async (req, res) => {
    try {
        ...
        const safeInfo = await getSafeInfo();
        // Create proposal
        const proposal = await createProposal('delete_council', req.session.address, safeInfo.threshold, { councilId });
        // Log audit
        await logAuditEvent('council_closed', req.session.address, 'council', councilId, { councilName: council.name, status: 'proposed' });
        // Notify signers
        await notifyAllSigners('vote_required', {
 ...
 */
router.post('/add-member', requireAuth, requireSafeOwner, validateBody(addMemberSchema), async (req, res) => {
    try {
        ...
        const safeInfo = await getSafeInfo();
        // Create proposal
        const proposal = await createProposal('add_member', req.session.address, safeInfo.threshold, {
            councilId,
        ...
        // Log audit
        await logAuditEvent('member_added', req.session.address, 'member', `${councilId}:${address}`, { councilName: council.name, memberName: name, status: 'proposed' });
        // Notify signers
        await notifyAllSigners('vote_required', {
 ...
 */
router.post('/remove-member', requireAuth, requireSafeOwner, validateBody(removeMemberSchema), async (req, res) => {
    try {
        ...
        const safeInfo = await getSafeInfo();
        // Create proposal
        const proposal = await createProposal('remove_member', req.session.address, safeInfo.threshold, {
            councilId,
            memberAddress: address,
        });
        // Log audit
        await logAuditEvent('member_removed', req.session.address, 'member', `${councilId}:${address}`, { councilName: council.name, status: 'proposed' });
        // Notify signers
        await notifyAllSigners('vote_required', {
 ...
 */
router.post('/:id/vote', requireAuth, requireSafeOwner, validateParams(proposalIdParamSchema), validateBody(voteSchema), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/:id/transaction', requireAuth, requireSafeOwner, validateParams(proposalIdParamSchema), async (req, res) => {
    try {
        const proposal = await getProposal(req.params.id);
        if (!proposal) {
            res.status(404).json({ error: 'Proposal not found' });
 ...
 */
router.post('/:id/execute', requireAuth, requireSafeOwner, validateParams(proposalIdParamSchema), async (req, res) => {
    try {
