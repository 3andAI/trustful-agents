...
- **Added `uploadToIPFS()` function** that uploads directly to Pinata (uses `VITE_PINATA_API_KEY` and `VITE_PINATA_SECRET_KEY`)
...
- **Added 3-second auto-refresh** with `staleTime: 0` and `refetchOnMount: 'always'` to show new claims immediately
...
- **COMPLETELY REWRITTEN** with agent lookup integrated:
  ...
  - Step 3: Evidence
    ...
    - **Uploads evidence directly to Pinata IPFS** (uses same credentials as provider dashboard)
    - Shows upload progress and success/error status
    - Falls back to manual URI entry if upload fails
...
11. **Evidence not uploaded to IPFS** - Added direct Pinata upload (uses same API key/secret as provider dashboard)
12. **Dashboard not showing new claims** - Added 3-second auto-refresh with `staleTime: 0`

---

## Environment Variables

Add to your `.env` file (same as provider dashboard):
...
Copy these from your provider dashboard's `.env` file. Without these, evidence files will not upload to IPFS (users can still provide manual IPFS/HTTP URIs).
